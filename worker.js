...
const EVENTS_HEADERS = [
  ...
  'Zones / Prices', 'Status', 'Booking Status', 'Thumbnail', 'Video URL', 'Category', 'Description', 'Announcement'
];
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (request.method === 'GET'   && pathname === '/videos')               return await handlePublicVideos(request, env, origin);
...
async function handleGet(request, env, origin) {
  ...
  if (!eventId) return corsResponse(JSON.stringify({ bookedSeats: [] }), 200, origin, true);
  ...
  if (!tabExists) return corsResponse(JSON.stringify({ bookedSeats: [], closed: false }), 200, origin, true);

  // Check if bookings are open for this event
  try {
    const evValues = await sheetsRead(env, token, EVENTS_TAB, 'A:M');
    if (evValues && evValues.length > 1) {
      ...
      if (evRow && bookingCol !== -1) {
        ...
        if (bookingStatus === 'closed') {
          return corsResponse(JSON.stringify({ bookedSeats: [], closed: true }), 200, origin, true);
        }
      }
    }
  ...
  if (!values || values.length <= 1) return corsResponse(JSON.stringify({ bookedSeats: [] }), 200, origin, true);
  ...
  if (seatsCol === -1) return corsResponse(JSON.stringify({ bookedSeats: [] }), 200, origin, true);
  ...
  return corsResponse(JSON.stringify({ bookedSeats: Array.from(bookedSet) }), 200, origin, true);
}
...
async function handlePost(request, env, origin) {
  ...
  if (data.website || data.company_name || data.fax) {
    // Silently accept but don't process — don't reveal to bots
    return corsResponse(JSON.stringify({ success: true, paynow: true, bookingRef: 'LSM-bot-0' }), 200, origin);
  }

  // ── IP RATE LIMIT — max 5 attempts per IP per 10 minutes ─────────────
  const clientIP = request.headers.get('cf-connecting-ip') ||
                   request.headers.get('x-forwarded-for') ||
                   'unknown';
  const ipKey    = `ratelimit:${clientIP}:${Math.floor(Date.now() / (10 * 60 * 1000))}`;
  if (env.BOOKING_RATE) {
    try {
      const count = Number(await env.BOOKING_RATE.get(ipKey) || 0);
      if (count >= 5) {
        return corsResponse(JSON.stringify({ success: false, error: 'Too many booking attempts. Please try again in 10 minutes.' }), 429, origin);
      }
      await env.BOOKING_RATE.put(ipKey, String(count + 1), { expirationTtl: 600 });
    } catch (e) { console.error('Rate limit KV error (non-fatal):', e); }
  }
  ...
  if (!/^[^\s@]+@[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}$/.test(String(data.email).trim())) {
  ...
  if (!/^[A-Za-z0-9,\s]{1,2000}$/.test(seatsRaw)) {
  ...
  // Sanity check — max 50 seats per booking
  if (seatsToBook.length === 0 || seatsToBook.length > 50) {
  ...
  if (values && values.length > 1) {
    ...
    const EXPIRE    = 24 * 60 * 60 * 1000; // 24 hours
    if (seatsCol2 !== -1) {
      for (let i = 1; i < values.length; i++) {
  ...
  // ── DOUBLE-CHECK: re-read sheet to catch race condition ──────────────
  // If two requests passed the conflict check simultaneously, both will have
  // written a row. We re-read and check if any OTHER row (different bookingRef)
  // also contains our seats with Pending Payment status.
  try {
    const verifyValues = await sheetsRead(env, token, eventId, 'A:I');
    if (verifyValues && verifyValues.length > 1) {
      const vHeader     = verifyValues[0];
      const vSeatsCol   = vHeader.indexOf('Seats');
      const vStatusCol  = vHeader.indexOf('Payment Status');
      const vRefCol     = Math.max(vHeader.indexOf('Booking Ref'), vHeader.indexOf('HitPay Ref'));
      const vTsCol      = vHeader.indexOf('Timestamp');
      const nowMs       = Date.now();
      const EXPIRE      = 24 * 60 * 60 * 1000;

      // Find any OTHER active pending row that has our seats
      const duplicateRow = verifyValues.slice(1).find(r => {
        const rowRef    = String(r[vRefCol]   || '').trim();
        const rowStatus = String(r[vStatusCol]|| '').trim();
        const rowSeats  = String(r[vSeatsCol] || '').trim();
        const rowTs     = String(r[vTsCol]    || '').trim();

        if (rowRef === bookingRef) return false; // skip our own row
        if (rowStatus !== 'Pending Payment') return false;

        // Skip expired rows
        if (rowTs) {
          const created = new Date(rowTs).getTime();
          if (!isNaN(created) && (nowMs - created) > EXPIRE) return false;
        }

        // Check if any of our seats appear in this other row
        const otherSeats = rowSeats.split(',').map(s => s.trim()).filter(Boolean);
        return seatsToBook.some(s => otherSeats.includes(s));
      });

      if (duplicateRow) {
        // Race condition detected — our row was written but so was another.
        // Delete our row by marking it cancelled so it doesn't block seats.
        // Find our row number and mark it Cancelled.
        const ourRowIdx = verifyValues.slice(1).findIndex(r =>
          String(r[vRefCol] || '').trim() === bookingRef
        );
        if (ourRowIdx !== -1 && vStatusCol !== -1) {
          const sheetId    = env.GOOGLE_SHEET_ID;
          const colLetter  = columnToLetter(vStatusCol + 1);
          const rowNum     = ourRowIdx + 2; // +1 for header, +1 for 1-index
          const rangeParam = encodeURIComponent(`${eventId}!${colLetter}${rowNum}`);
          await fetch(
        ...
        const dupSeats = seatsToBook.filter(s =>
          String(duplicateRow[vSeatsCol] || '').split(',').map(x => x.trim()).includes(s)
        );
        return corsResponse(JSON.stringify({
          success: false, conflict: true, conflictSeats: dupSeats,
          message: 'These seats were just taken by someone else. Please choose other seats.'
        }), 200, origin);
      }
    }
  } catch (verifyErr) {
    console.error('Post-write verify failed (non-fatal):', verifyErr);
    // Non-fatal — booking proceeds; race condition would need manual resolution
  }
  // ── END DOUBLE-CHECK ─────────────────────────────────────────────────
  ...
  // Generate dynamic PayNow QR string (locked amount + booking ref)
  let paynowQR = '';
  try {
    // QR expiry: 24h from now, formatted YYYYMMDD
    const expDate = new Date(Date.now() + 24 * 60 * 60 * 1000);
    const expiry = `${expDate.getFullYear()}${String(expDate.getMonth() + 1).padStart(2, '0')}${String(expDate.getDate()).padStart(2, '0')}`;
    // Booking ref can be long; PayNow reference max ~25 chars. Use a trimmed ref.
    const shortRef = bookingRef.replace(/[^A-Za-z0-9]/g, '').slice(-20);
    paynowQR = buildPayNowQR({
      uen:    env.PAYNOW_UEN || '53384102W',
      amount: totalPayable,
      ref:    shortRef,
      expiry
    });
  } catch (qrErr) {
    console.error('PayNow QR generation failed (non-fatal):', qrErr);
  }
...
// ============================================================
// PAYNOW DYNAMIC QR — EMVCo / SGQR-compliant string builder
// Builds the QR payload string per SGQR spec (ID 26 = SG.PAYNOW),
// with locked amount and embedded booking reference, plus CRC16.
...
function crc16ccitt(str) {
  // CRC-16/CCITT-FALSE — required by EMVCo (ID "63")
  let crc = 0xFFFF;
...
// Builds a dynamic PayNow QR payload string.
// uen: merchant UEN; amount: number; ref: booking reference (uneditable);
// merchantName: trade name; expiry: 'YYYYMMDD' or null.
function buildPayNowQR({ uen, amount, ref, merchantName = 'LIGHT AND SHADOW MEDIA', expiry = null }) {
  // Merchant Account Information — ID "26" (SG.PAYNOW)
  let merchantAcct =
      emvField('00', 'SG.PAYNOW') +   // Globally Unique Identifier
      emvField('01', '2') +           // Proxy type: 2 = UEN
      emvField('02', uen) +           // Proxy value: the UEN
      emvField('03', '0');            // Editable amount: 0 = NOT editable (locked) per SGQR spec A.2.15
  if (expiry) merchantAcct += emvField('04', expiry); // QR expiry YYYYMMDD

  // Additional Data Field Template — ID "62", reference under sub-ID "01"
  const additionalData = emvField('01', ref);

  // Amount formatted to 2 decimals
  const amt = Number(amount).toFixed(2);

  // Assemble payload (without CRC yet)
  let payload =
      emvField('00', '01') +                 // Payload Format Indicator
      emvField('01', '12') +                 // Point of Initiation: 12 = dynamic
      emvField('26', merchantAcct) +         // Merchant Account Info (PayNow)
      emvField('52', '0000') +               // Merchant Category Code
      emvField('53', '702') +                // Currency: 702 = SGD
      emvField('54', amt) +                  // Transaction Amount (locked)
      emvField('58', 'SG') +                 // Country
      emvField('59', merchantName.slice(0, 25)) + // Merchant Name (max 25)
      emvField('60', 'Singapore') +          // Merchant City
      emvField('62', additionalData);        // Additional Data (reference)

  // Append CRC field id+len ("6304") then compute checksum over everything incl. "6304"
  payload += '6304';
...
<table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#fff;border-radius:8px;overflow:hidden;">
  ...
  <tr><td style="padding:32px;">
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="background:#f9f9f9;border:1px solid #e5e5e5;border-radius:8px;">
      <tr><td style="padding:20px;">
        <p style="margin:0 0 8px;color:#333;"><strong>(1)</strong> PayNow to QR Code from the website or UEN: <strong style="color:#c9a227;font-size:1.3rem;">53384102W</strong><br><span style="color:#888;font-size:0.8rem;">LIGHT AND SHADOW MEDIA</span></p>
        <p style="margin:12px 0 8px;color:#333;"><strong>(2)</strong> Add reference / မှတ်ချက်: <strong style="color:#c9a227;font-size:1.1rem;">${seatList}</strong></p>
        <p style="margin:0 0 12px;color:#333;"><strong>(3)</strong> Send payment screenshot  <strong>by replying to this email.</strong>.</p>
  ...
  <tr><td style="background:#0a0a0c;padding:24px 32px;">
    ...
    <p style="color:#ccc;font-size:0.85rem;margin:0;"><strong style="color:#c9a227;">(3)</strong> Payment Screentshot ကို ဤ email သို့ reply ပြန်၍ပို့ပေးပါ။ ၂၄ နာရီအတွင်း E-ticket ပို့မည်။</p>
  </td></tr>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="background:#fff;border:1px solid #e5e5e5;border-radius:8px;margin-bottom:16px;overflow:hidden;">
      ...
      <tr>
        <td colspan="2" style="background:#f9f5e7;padding:10px 20px;border-top:1px solid #e5e5e5;">
          <span style="font-size:0.75rem;color:#888;">Seats: ${seat}</span>
        </td>
      </tr>
    </table>
  `).join('');
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="background:#f9f9f9;border:1px solid #e5e5e5;border-radius:8px;">
        <tr><td style="padding:16px 20px;">
          <div style="font-size:0.85rem;color:#555;line-height:1.6;">
            ...
            <strong style="color:#111;">Need help?</strong> Contact us by email  at ${fromEmail}
          </div>
        </td></tr>
      </table>
    </td>
...
// ============================================================
// GET /videos?page=corporate|film — public endpoint
// Returns videos from the Videos tab, filtered by page
// Videos tab columns: ID | Title | Category | Year | Client/Role | Description | Drive Link | Thumbnail | Page
// ============================================================
async function handlePublicVideos(request, env, origin) {
  const url      = new URL(request.url);
  const pageFilter = (url.searchParams.get('page') || '').trim().toLowerCase();

  const token     = await getAccessToken(env);
  const sheetMeta = await getSheetMeta(env, token);
  const tabExists = sheetMeta.sheets.some(s => s.properties.title === 'Videos');
  if (!tabExists) return corsResponse(JSON.stringify({ videos: [] }), 200, origin, true);

  const values = await sheetsRead(env, token, 'Videos', 'A:I');
  if (!values || values.length <= 1) return corsResponse(JSON.stringify({ videos: [] }), 200, origin, true);
  ...
  const clientCol = header.indexOf('Client/Role');
  ...
  const videos = values.slice(1)
    .filter(row => row[idCol] && String(row[idCol]).trim())
    .map(row => ({
      id:          String(row[idCol]     || '').trim(),
      title:       String(row[titleCol]  || '').trim(),
      category:    String(row[catCol]    || '').trim().toLowerCase(),
      year:        String(row[yearCol]   || '').trim(),
      client:      clientCol !== -1 ? String(row[clientCol] || '').trim() : '',
      description: String(row[descCol]   || '').trim(),
      driveLink:   String(row[linkCol]   || '').trim(),
      thumbnail:   String(row[thumbCol]  || '').trim(),
      page:        pageCol !== -1 ? String(row[pageCol] || '').trim().toLowerCase() : '',
    }))
    .filter(v => !pageFilter || v.page === pageFilter);

  return corsResponse(JSON.stringify({ videos }), 200, origin, true);
}
...
async function handlePublicEvents(request, env, origin) {
  ...
  const values = await sheetsRead(env, token, EVENTS_TAB, 'A:M');
  ...
  const descCol         = header.indexOf('Description');
  const announcementCol = header.indexOf('Announcement');
  ...
  const events = values.slice(1)
    ...
    .map(row => ({
      ...
      description:    String(row[descCol]     || '').trim(),
      announcement:   announcementCol !== -1 ? String(row[announcementCol] || '').trim() : '',
    }));
...
async function handleAdminAddEvent(request, env, origin) {
  ...
  const existing = await sheetsRead(env, token, EVENTS_TAB, 'A:M');
...
// CORS
// ============================================================
function corsResponse(body, status = 200, origin = CORS_ORIGIN, noCache = false) {
  const headers = {
    'Content-Type':                 'application/json',
